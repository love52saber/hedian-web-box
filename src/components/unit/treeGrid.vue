    ...
    <table class="table table-bordered" id='hl-tree-table'>
      ...
      <tbody>
        ...
        <tr v-for="(item,index) in initItems" :key="item.id" v-show="show(item)" :class="{'child-tr':item.parent}">
          <td :title="renderBody(item,column)" v-for="(column,snum) in columns" :key="column.key" :style="tdStyle(column, snum)">
            <label v-if="column.type === 'selection'">
              <input type="checkbox" :value="item.id" v-model="checkGroup" @click="handleCheckClick(item,$event,index)">
            </label>
            <div v-if="column.type === 'action'">
              <Icon :style="action.style" style="cursor:pointer" :type="action.type" :size="action.size || '20'" color="#04bbb7" @click="RowClick(item,$event,index,action.text)" v-for='action in (column.actions)' :key="action.text" :title="action.text" />
            </div>
            <label class="text2" @click="toggle(index,item)" v-if="!column.type">
              <span v-if='snum==iconRow()'>
                <i v-html='item.spaceHtml'></i>
                <i v-if="item.children&&item.children.length>0" style="font-size:16px;line-height:20px;width:15px;cursor:pointer" class="ivu-icon" :class="{'ivu-icon-md-return-right':!item.expanded,'ivu-icon-ios-arrow-down':item.expanded }"></i>
...
export default {
  ...
  methods: {
    ...
    toggle (index, item) {
      ...
      let spaceHtml = ''
      for (var i = 1; i < level; i++) {
        spaceHtml += "<i class='ms-tree-space' style='margin-right:20px'></i>"
      }
      if (item.children) {
        if (item.expanded) {
    ...
    renderBody (row, column, index) {
      ...
      return this.format(column.key, row[column.key])
    },
    format (key, val) {
      if (key === 'orgType') {
        if (val === 1) {
