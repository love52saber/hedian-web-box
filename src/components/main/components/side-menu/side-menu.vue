<template>
  <div class="side-menu-wrapper">
    ...
    <div class="menu-collapsed" v-show="collapsed" :list="menuList">
      <template v-for="item in menuList">
        <collapsed-menu v-if="item.children && item.children.length > item.meta.overNumber" @on-click="handleSelect" hide-title :root-icon-size="rootIconSize" :icon-size="iconSize" :theme="theme" :parent-item="item" :key="`drop-menu-${item.name}`"></collapsed-menu>
...
export default {
  ...
  props: {
    menuList: {
      type: Array,
      default () {
        console.log('11111', this.menuList)
        return []
      }
    },
    collapsed: {
